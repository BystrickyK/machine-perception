...
def advanced_lut(img, p_low, p_high, verbose=False, plot=False, segment_borders=None, segment_count=10):
    ...
    and dark parts of the image and do histogram equalization separately.
    ...
    for seg, thresholds in enumerate(segment_borders):
        ...
        if plot:
            plt.figure()
            x = [*range(thresholds[0], thresholds[1])]
            plt.plot(x, lookup_table[thresholds[0]:thresholds[1]])
...
# transformed_img_5 = advanced_lut(img, P_LOW, P_HIGH, verbose=True, segment_count=5)

# stretch the histogram within the defined segments
borders = ((0, 104), (105, 255))  # values set up for the bio-medical img
...
transformed_img_border = advanced_lut(img, P_LOW, P_HIGH, segment_borders=((5, 240),), verbose=False, plot=False)
...
transformed_img_masked_clahe[:, :, 0] = masked_clahe(
    transformed_img_masked_clahe[:, :, 0])  # local histogram equalization
