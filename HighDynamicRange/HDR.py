...
def get_weights(Z, L, aggro):
    """

    Args:
        Z: Pixels
        L: Intensity range
        aggro: How aggresively do the weights punish values far from the center, value must be >=1

    Returns:

    """
    return np.interp(Z, [0, (L - 1) / 2, L - 1], [1 / aggro, 1, 1 / aggro])
...
def calc_weights(imgs, L=256, aggro=1000):
    """
...
def visualize_exposure(E, t, dim):
    ...
    axs[1].set_title("Exposure times (sorted)")
...
def estimate_response(Z, t, weights, lambda_=2):
    ...
    # b = t.flatten()
    # eps = np.nextafter(0, 1)  # tiny number, size of 'minimal' step from 0 to 1
    # b = np.log(b + eps)  # added eps to avoid infinities when t==0
    # b = np.append(b, 0)  # From constraint (exposure time in img 1 is 1 sec)
    # b = b * weights
    ...
    # Add system A of LHS equations from (6) -> g(Zij) - log(Ei)
    ...
    d2_indexes_diag = d2_indexes_subdiag + np.array([0, 1])
...
# %%
# Load images
dim, imgs, t = read_images('./imgs/lampicka*.jpg', 20)
...
imgs_weights = calc_weights(imgs_L, aggro=5)  # values at the intensity limits have weights 1/aggro
# plot_montage(imgs_weights * 255, title_str='weighted images')

weights = imgs_weights.flatten()
Z = imgs_to_Z(imgs_L)
E, finv = estimate_response(Z, t, weights, lambda_=100000)
# w = np.ones(weights.shape)
# E, finv = estimate_response(Z, t, w, lambda_=100000)

# visualize_exposure(E, t, dim)
# Plot the irradiances in log scale== 1/aggro
x_img = np.reshape(E, np.flip(dim))  # Vector of pixels to 2D array
fig, axs = plt.subplots(1, 2, tight_layout=True)
